...
//	}
	...
	public String getBookingsForThisWeek() {
		...
		List<Booking> bookings = bookingRepo.findAll().stream()
				.filter(b -> (!b.getBeginTime().before(monday)) && (!b.getEndTime().after(friday)))
				.collect(Collectors.toList());
		return Views.getWeeklyBookingsPage(bookings);
	}

	private String checkBooking(Booking booking) {
		return checkBooking(booking, bookingRepo);
	}
	
	@SuppressWarnings("deprecation")
	static String checkBooking(Booking booking, BookingRepo bookingRepo) {
	...
	private static boolean isOverlapping(Booking existingBooking, Booking newBooking) {
		...
		if ((!newBooking.getBeginTime().after(existingBooking.getBeginTime()))
				&& (newBooking.getEndTime().after(existingBooking.getBeginTime()))) {
			return true;
		}
		// new.begin >= existing.begin && new.begin < existing.end
		if (!newBooking.getBeginTime().before(existingBooking.getBeginTime())
				&& (newBooking.getBeginTime().before(existingBooking.getEndTime()))) {
			return true;
		}
		return false;
	}
